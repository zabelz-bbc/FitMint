...
<?php endif;?>
		...
		for($k = 0; $k < count ( $data3 ); $k += 1) {
			...
			if (isset($_SESSION['loggedInUserId']) && $data3 [$k]->getBenutzer_id () == $_SESSION ['loggedInUserId']) {
				...
				echo '<a href="/kommentar/doShowKommentar?kommentarId=' . $data3 [$k]->getId () . '&benutzerId=' . $data3 [$k]->getBenutzer_id () . '"<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></a>';				
			}
			echo "<br>";
			}
			?>
    ...
    <?php echo $data2->getTitel(); ?> </h2>
		...
		for($l = 0; $l < count ( $data4 ); $l += 1) {
			...
			if (isset($_SESSION['loggedInUserId']) && $data4 [$l]->getBenutzer_id () == $_SESSION ['loggedInUserId']) {
				...
				echo '<a href="/kommentar/doShowKommentar?kommentarId=' . $data4 [$l]->getId () . '&benutzerId=' . $data4 [$l]->getBenutzer_id () . '"<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></a>';
			}
			echo "<br>";
		}
		?>
